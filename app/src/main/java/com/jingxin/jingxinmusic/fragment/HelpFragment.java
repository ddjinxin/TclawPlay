...
public class HelpFragment extends BaseFloatFragment {
    ...
    private void buildHelpContent() {
        ...
        // ========== 二、主题与风格 ==========
        addSection("二、主题与风格");
        addPara("右上角两个按钮分别控制日夜模式和配色风格：");
        addBullet("主题按钮（太阳/月亮图标）：", "切换白天/夜间模式。");
        addBullet("风格按钮（调色板图标）：", "循环切换四种配色风格 — 春意盎然、蔚蓝天地、万紫千红、高级灰。");
        addPara("");
        addPara("每种风格在白天和夜间模式下有不同配色，歌词高亮色和界面点缀色会随之变化。高级灰日间模式下歌词高亮色为红色。");
        ...
        addBullet("主题按钮 — 切换日夜模式");
        addBullet("风格按钮 — 切换配色风格");
        addPara("");
        addPara("播放页：");
